...
public class IdQuery {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println(
                "-input <path>设置数据源文件路径（必需）；初次运行添加参数-init <num>生成num条流数据，每条数据大概40B；-output <path>设置sink文件夹(可选)；-para <num>设置并行数（可选)");*/
        ...
        SingleOutputStreamOperator<Tuple4<Long, Integer, Double, Double>> reducedStream =
                Source.assignTimestampsAndWatermarks(
                        ...
                        }).flatMap(new Tokenizer()).filter(new FilterFunction<Tuple4<Long, Integer, Double, Double>>() {
                    @Override public boolean filter(Tuple4<Long, Integer, Double, Double> longIntegerDoubleDoubleTuple4)
                            throws Exception {
        ...
        System.out.println(
                streamN + "条记录文件1000s长度滑动窗口，100s滑动一次，过滤后仅保存ID在100-200之间的记录，" + env.getParallelism() + "线程" + sink
                        + "总耗时" + (endTime - startTime));
    }

    public static final class Tokenizer implements FlatMapFunction<String, Tuple4<Long, Integer, Double, Double>> {

        @Override public void flatMap(String value, Collector<Tuple4<Long, Integer, Double, Double>> out) {
            streamN++;
    ...
    public class CountWindowAverage extends RichFlatMapFunction<Tuple2<Long, Long>, Tuple2<Long, Long>> {
        ...
        @Override public void flatMap(Tuple2<Long, Long> input, Collector<Tuple2<Long, Long>> out) throws Exception {
            // 获取当前状态值
