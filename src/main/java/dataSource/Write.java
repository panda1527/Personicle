...
public class Write {
    ...
    public static class City {
        ...
        @Override public String toString() {
            return "id=" + id + ", isCenter=" + isCenter + ", lon=" + lon + ", lat=" + lat + ", nearestCenter="
                    + nearestCenter;
        }
    }
    ...
    public static class Order {
        ...
        public Order(int id, int type, int start_time1, int start_city, int end_city, int buyer, int seller,
                boolean done) {
            this.id = id;
            this.type = type;
        ...
        @Override public String toString() {
            return "id=" + id + ", type=" + type + ", start_time1=" + start_time1 + "path=" + start_city + "-"
                    + first_transfer + "-" + last_transfer + "-" + end_city + ", buyer=" + buyer + ", seller=" + seller;
        }
    }
    ...
    public static Order[] createOrders(City[] cities, int num) throws IOException {
        ...
        for (int i = 0; i < num; i++) {
            ...
            orders[i].arrive_time1 = orders[i].start_time1 + (int) (
                    dis(cities[orders[i].start_city].lon, cities[orders[i].start_city].lat,
                            cities[orders[i].first_transfer].lon, cities[orders[i].first_transfer].lat) / speed);
            orders[i].start_time2 = 2 * orders[i].arrive_time1 - orders[i].start_time1;
            orders[i].arrive_time2 = orders[i].start_time2 + (int) (
                    dis(cities[orders[i].first_transfer].lon, cities[orders[i].first_transfer].lat,
                            cities[orders[i].last_transfer].lon, cities[orders[i].last_transfer].lat) / speed);
            orders[i].start_time3 = 2 * orders[i].arrive_time2 - orders[i].start_time2;
            orders[i].arrive_time3 = orders[i].start_time3 + (int) (
                    dis(cities[orders[i].last_transfer].lon, cities[orders[i].last_transfer].lat,
                            cities[orders[i].end_city].lon, cities[orders[i].end_city].lat) / speed);
                ...
                for (int i = 0; i < len; i++) {
                    if (!orders[i].done) {
                        if (time < orders[i].start_time1) {
                            ...
                            lon = cities[orders[i].start_city].lon +
                                    (cities[orders[i].first_transfer].lon - cities[orders[i].start_city].lon) * (time
                                            - orders[i].start_time1) / (orders[i].arrive_time1 - orders[i].start_time1);
                            lat = cities[orders[i].start_city].lat +
                                    (cities[orders[i].first_transfer].lat - cities[orders[i].start_city].lat) * (time
                                            - orders[i].start_time1) / (orders[i].arrive_time1 - orders[i].start_time1);
                            Buff.write((time + " " + i + " " + lon + " " + lat + "\r\n").getBytes());
                            ...
                            lon = cities[orders[i].first_transfer].lon +
                                    (cities[orders[i].last_transfer].lon - cities[orders[i].first_transfer].lon) * (time
                                            - orders[i].start_time2) / (orders[i].arrive_time2 - orders[i].start_time2);
                            lat = cities[orders[i].first_transfer].lat +
                                    (cities[orders[i].last_transfer].lat - cities[orders[i].first_transfer].lat) * (time
                                            - orders[i].start_time2) / (orders[i].arrive_time2 - orders[i].start_time2);
                            Buff.write((time + " " + i + " " + lon + " " + lat + "\r\n").getBytes());
                            ...
                            lon = cities[orders[i].last_transfer].lon +
                                    (cities[orders[i].end_city].lon - cities[orders[i].last_transfer].lon) * (time
                                            - orders[i].start_time3) / (orders[i].arrive_time3 - orders[i].start_time3);
                            lat = cities[orders[i].last_transfer].lat +
                                    (cities[orders[i].end_city].lat - cities[orders[i].last_transfer].lat) * (time
                                            - orders[i].start_time3) / (orders[i].arrive_time3 - orders[i].start_time3);
                            Buff.write((time + " " + i + " " + lon + " " + lat + "\r\n").getBytes());
                        } else {
                            orders[i].done = true;
